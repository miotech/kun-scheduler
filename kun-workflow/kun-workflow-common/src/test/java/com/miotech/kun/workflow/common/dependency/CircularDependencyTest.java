...
public class CircularDependencyTest extends CommonTestBase {
    ...
    public void testFullUpdateTaskWithCircularDependencyShouldThrowsException() {
        ...
        Exception ex = assertThrows(IllegalArgumentException.class, () -> taskService.fullUpdateTaskById(task1.getId(), updateTask1));
        assertThat(ex.getMessage(), is("create task:" + task1.getId() + ", taskName:" + task1.getName() + ",with circular dependencies:" +
    ...
    public void testFullUpdateTaskWithSelfDependencyShouldThrowsException() {
        ...
        Operator op = MockOperatorFactory.createOperatorWithId(operatorId);
        operatorDao.createWithId(op, operatorId);
        Exception ex = assertThrows(IllegalArgumentException.class, () -> taskService.fullUpdateTaskById(task.getId(), updateTask1));
        assertThat(ex.getMessage(), is("create task:" + task.getId() + ", taskName:" + task.getName() + ",with circular dependencies:" +
    ...
    public void testUpdateTaskWithCircularDependencyShouldThrowsException() {
        ...
        Exception ex = assertThrows(IllegalArgumentException.class, () -> taskService.partialUpdateTask(task1.getId(), updateTask1));
        assertThat(ex.getMessage(), is("create task:" + task1.getId() + ", taskName:" + task1.getName() + ",with circular dependencies:" +
                taskList.stream().map(Task::getId).sorted(Comparator.reverseOrder()).collect(Collectors.toList())));
