...
public class DeployedTaskController {
    ...
    public RequestResult<List<TaskRun>> fetchTaskRunWithAllFinishedDownstream(@PathVariable Long taskRunId) {
        Preconditions.checkArgument(Objects.nonNull(taskRunId), "task run id cannot be null");
        List<TaskRunStatus> filterStatus = Arrays.asList(TaskRunStatus.SUCCESS, TaskRunStatus.FAILED, TaskRunStatus.CHECK_FAILED);
