...
public class TaskDefinitionService extends BaseSecurityService implements TaskDefinitionFacade {
    ...
    public List<TaskTry> runBatch(TaskTryBatchRequest taskTryBatchRequest) {
        ...
        //check whether circular dependency
        ...
        while (iterator.hasNext()) {
            ...
            List<TaskDependency> dependencies = graph.incomingEdgesOf(taskDef).stream()
                    ...
                    .map(taskDefToTaskMap::get).filter(Objects::nonNull)
                    .map(Task::getId)
            ...
            logger.debug("Try to save task, TaskName={}", task.getName());
            task = workflowClient.saveTask(task, searchTags);
            taskDefToTaskMap.put(taskDef, task);
            taskIdToTaskDefIdMap.put(task.getId(), taskDefId);
        }


        //prepare RunTaskRequest
        RunTaskRequest runTaskRequest = new RunTaskRequest();
        taskDefToTaskMap.values().stream()
                .map(Task::getId)
        ...
        //persist task try
        return persistTaskTry(taskIdToTaskRunMap, taskDefToTaskMap, taskIdToTaskDefIdMap);
    }


    private Graph<TaskDefinition, DefaultEdge> buildTaskRunBatchGraph(TaskTryBatchRequest taskTryBatchRequest) {
        ...
        for (TaskDefinition taskDef : taskDefList) {
            //get upstream taskdef id which in this batch
    ...
    private List<TaskTry> persistTaskTry(Map<Long, TaskRun> taskIdToTaskRunMap,
        ...
        for (TaskRun taskRun : taskIdToTaskRunMap.values()) {
            Long taskTryId = DataPlatformIdGenerator.nextTaskTryId();
            Long taskDefId = taskIdToTaskDefIdMap.get(taskRun.getTask().getId());
            TaskDefinition taskDef = taskDefToTaskMap.keySet().stream()
    ...
    public TaskDefinitionVO convertToVO(TaskDefinition taskDefinition) {
        ...
        List<TaskCommitVO> taskCommitVOs = taskCommitService.findByTaskDefinitionId(taskDefinition.getDefinitionId()).stream()
                .map(taskCommitService::convertVO).collect(Collectors.toList());
        return mapVO(taskDefinition, commitStatus, latestCommits, taskCommitVOs);
    }
    ...
    public List<TaskDefinitionVO> convertToVOList(List<TaskDefinition> taskDefinitions, boolean withTaskPayload) {
        ...
        return taskDefinitions.stream()
                .map(taskDefinition -> mapVO(taskDefinition, commitStatus, null, withTaskPayload, latestCommits, Collections.emptyList()))
                .collect(Collectors.toList());
    }

    private TaskDefinitionVO mapVO(TaskDefinition taskDefinition, Map<Long, Boolean> commitStatus, Map<Long, TaskCommit> latestCommits, List<TaskCommitVO> taskCommitVOs) {
        List<Long> upstreamDefinitionIds = resolveUpstreamTaskDefIds(taskDefinition.getTaskPayload());
        return mapVO(taskDefinition, commitStatus, taskDefinitionDao.fetchByIds(upstreamDefinitionIds), true, latestCommits, taskCommitVOs);
    }
    ...
    private TaskDefinitionVO mapVO(TaskDefinition taskDefinition, Map<Long, Boolean> commitStatus, List<TaskDefinition> taskDefinitions,
                                   boolean withTaskPayload, Map<Long, TaskCommit> latestCommits, List<TaskCommitVO> taskCommits) {
