...
export var eventProps = (function() {
  ...
  []
    .concat(
      _toConsumableArray(mouseEvents),
      _toConsumableArray(touchEvents),
      _toConsumableArray(keyboardEvents),
    )
...
export var isPageIndex = function isPageIndex(props, propName, componentName) {
  ...
  if (isDefined(pageIndex)) {
    if (typeof pageIndex !== 'number') {
      return new Error(
        '`'
          ...
          .concat(_typeof(pageIndex), '` supplied to `')
          .concat(componentName, '`, expected `number`.'),
      );
    }
    ...
    if (pageIndex + 1 > numPages) {
      return new Error(
        'Expected `'.concat(propName, '` to be less or equal to ').concat(numPages - 1, '.'),
      );
    }
  } else if (!isDefined(pageNumber)) {
    return new Error(
      '`'
        .concat(propName, '` not supplied. Either pageIndex or pageNumber must be supplied to `')
  ...
  if (isDefined(pageNumber)) {
    if (typeof pageNumber !== 'number') {
      return new Error(
        '`'
          .concat(propName, '` of type `')
          .concat(_typeof(pageNumber), '` supplied to `')
          .concat(componentName, '`, expected `number`.'),
      );
    }
    ...
    if (pageNumber > numPages) {
      return new Error(
        'Expected `'.concat(propName, '` to be less or equal to ').concat(numPages, '.'),
      );
    }
...
export var isPdf = PropTypes.oneOfType([
  PropTypes.shape({
