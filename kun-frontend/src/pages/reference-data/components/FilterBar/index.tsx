...
export interface Filters extends Omit<FetchRdmDatasParams, 'pageSize' | 'pageNumber'> {}
...
const FilterBar: React.FC<Props> = ({ filters, totalCount, onFilterChange }) => {
  const t = useI18n();
  // Fetch glossary list
  const { data: glossaryOptions } = useRequest(fetchGlossaryByOwners.bind(null, filters.resourceAttributeMap?.owners), {
    refreshDeps: [filters.resourceAttributeMap?.owners],
  });

  // Fetch owner list
  const { data: ownerOptions } = useRequest(
    fetchOwnersByGlossary.bind(null, filters.resourceAttributeMap?.glossaries),
    {
      refreshDeps: [filters.resourceAttributeMap?.glossaries],
    },
  );
    ...
    (
      ...
      switch (filterName) {
        ...
        case 'glossaries':
          updatedFilters = {
            resourceAttributeMap: {
              owners: filters.resourceAttributeMap?.owners,
              glossaries: value ? [JSON.parse(value as string)] : undefined,
            },
          };
          break;
        case 'owners':
          updatedFilters = {
            resourceAttributeMap: {
              glossaries: [...(filters.resourceAttributeMap?.glossaries || [])],
