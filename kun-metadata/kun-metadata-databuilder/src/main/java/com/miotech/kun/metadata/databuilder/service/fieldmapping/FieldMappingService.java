...
public class FieldMappingService {
    ...
    private static Cache<Long, List<DatasetFieldMapping>> localCache = CacheBuilder.newBuilder()
            .maximumSize(1024)
    ...
    public DatasetFieldType.Type parse(long datasourceId, String rawType) {
        List<DatasetFieldMapping> fieldMappingsCache = localCache.getIfPresent(datasourceId);
        if (CollectionUtils.isEmpty(fieldMappingsCache)) {
            List<DatasetFieldMapping> fieldMappings = fieldMappingDao.fetchByDatasourceId(datasourceId);
            localCache.put(datasourceId, fieldMappings);
