...
public class DataQualityRepository extends BaseRepository {
    ...
    private Map<Long, List<AbnormalTask>> getFailedTask(List<Long> datasetGids) {
        String sql = "select kdad.task_id, kdad.task_name, kdad.task_run_id, kdad.update_time, kdad.dataset_gid " +
                "from (select task_id, task_name, task_run_id, update_time, dataset_gid, status, row_number() over(partition by dataset_gid order by update_time desc) as rn from kun_dq_abnormal_dataset " +
                "where dataset_gid in " + toColumnSql(datasetGids.size()) + " " + ") kdad " +
                "where kdad.rn = 1 and kdad.status is not null ";
        return jdbcTemplate.query(sql, rs -> {
