...
public class EmailService {
    ...
    public void sendEmail(String subject, String message, EmailNotifierUserConfig userConfig) {
        if (!enabled) {
            return;
        }
    ...
    private EmailContent parseEventToContent(Event event) {
        if (event instanceof TaskAttemptStatusChangeEvent) {
            ...
            Optional<Long> taskDefinitionId = deployedTaskFacade.findByWorkflowTaskId(e.getTaskId()).map(deployedTask -> deployedTask.getDefinitionId());
