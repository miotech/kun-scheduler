...
public class SlaDao {
    ...
    private Pair<String, Object[]> buildSaveSQL(TaskDefinitionNode taskDefinitionNode) {
        ...
        if (taskDefinitionNode.getRunTime() != null) {
            params.add("runTime: ?");
            values.add(taskDefinitionNode.getRunTime());
        }
        ...
        return jdbcTemplate.query(query, rs -> {
            ...
            while (rs.next()) {
                ...
                List<TaskDefinitionNode> collect = nodes.stream().map(node -> {
                    ...
                    if (node.containsKey("runTime")) {
                        taskDefinitionNode.setRunTime((Integer) node.get("runTime"));
                    }
    ...
    public void updateRunTime(Long definitionId, int runTime) {
        String query = "MATCH (td: TASK_DEFINITION{id: ?}) SET td.runTime = ? RETURN td";
        jdbcTemplate.update(query, definitionId, runTime);
    }
    ...
    public TaskDefinitionNode findById(Long taskDefId) {
        ...
        return jdbcTemplate.query(query, rs -> {
            ...
            if (rs.next()) {
                ...
                if (nodeMap.containsKey("runTime")) {
                    taskDefinitionNode.setRunTime(((Long) nodeMap.get("runTime")).intValue());
                }
            }
            return taskDefinitionNode;
        }, taskDefId);
